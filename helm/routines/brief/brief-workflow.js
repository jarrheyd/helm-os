...
const WATCH_TEXT = (DISCORD.watchPosts || []).map(w => `"${w.label}" (channel ${w.channelId}, author ${w.authorId})`).join('; ')
...
// Boards the MCP cannot read for this user (wrong account, no connector) set readVia:'browser' + browserUrl.
const BROWSER_BOARDS = BOARDS.filter(b => b.readVia === 'browser' && b.browserUrl)
const BROWSER_BOARDS_TEXT = BROWSER_BOARDS.map(b => `- ${b.connector.toUpperCase()} (${(b.projectKeys||[]).join(', ')}) READS THROUGH THE BROWSER, NOT THE MCP${b.note ? ' ('+b.note+')' : ''}: an MCP search there returns nothing and proves nothing. Open ${b.browserUrl} in a NEW tab of the logged-in Chrome, list tabs to get that tab's id, switch back to whatever tab was in front before (the owner may be in a call), then read page content WITH that tab id set. Never read page content without a tab id: it reads the front tab (a 2026-09-17 run read the calendar instead of the board). Parse notifications newer than the window; approvals added to ${OWNER}, assignments and @mentions are items. Browser unreachable = fall back to a second browser connector with an explicit tab id, then to that tool's notification emails.`).join('\n')
const ORG = (C.identity && C.identity.org) || 'the team'
const TEAM_DOMAIN = (C.identity && C.identity.teamDomain) || ''
const ALIAS_PATTERN = (C.connectors && C.connectors.projectAliasPattern) || ''
const TEAM_ADDR = TEAM_DOMAIN ? '@' + TEAM_DOMAIN : ORG + ' address'
...
VOICE CHECK (2026-09-25): before any chat or email draft is written or surfaced, run node "${VAULT}/.helm/helm/routines/voice/check.js" --channel <whatsapp|telegram|discord|email|gchat|teams> --to "<recipient>" "<draft>". Exit 2 means it is out of ${OWNER}'s range in that channel: rewrite from his last 10-20 sends in that exact thread and check again. The live sample rule below still applies; the check does not replace it.
...
// BALL: ball-in-court on a client project (2026-09-16: a client reply on a project group alias, addressed to a
// teammate, sat a week because the sweep dropped it as "not his"). One entry per project where the ball is on the
// team, read from the CLIENT venue (the project alias thread / client domain / client chat), never inferred off an
// internal channel. Teammate-owned balls render in BY AREA; only yours:true (the owner himself owes it) reaches NEEDS YOU.
const BALL = { type: 'object', required: ['area', 'owes', 'owner', 'since', 'venue'], properties: {
  area: { type: 'string' },   // the project
  owes: { type: 'string' },   // what the team owes the client, plain and specific
  owner: { type: 'string' },  // who on the team holds it (a teammate name, or the owner)
  since: { type: 'string' },  // when the ball landed on the team (a date)
  venue: { type: 'string' },  // the client thread/alias this was actually read from
  yours: { type: 'boolean' } } }  // true only if the owner himself owes the next move

const EMAIL_SCHEMA = { type: 'object',
  required: ['unread_total', 'marked_read', 'items', 'ball_on_us', 'read_summary', 'decisions', 'drafts_created', 'ledger_rows_written', 'failures'],
  properties: { unread_total: { type: 'number' }, marked_read: { type: 'number' },
    items: { type: 'array', items: ITEM },
    ball_on_us: { type: 'array', items: BALL },
...
const CHATS_SCHEMA = { type: 'object',
  required: ['channels_swept', 'channels_failed', 'items', 'ball_on_us', 'channel_digests', 'direct_tags', 'decisions', 'ledger_rows_written', 'failures'],
  properties: { channels_swept: { type: 'array', items: { type: 'string' } },
    ...
    // direct_tags (2026-09-14, he flagged 41 Discord tags in one day with ~6 surfaced): EVERY message in the window
    // that @-tags him on any chat surface, one row each, with whether he has answered since. Never digested away.
    direct_tags: { type: 'array', items: { type: 'object', required: ['when', 'channel', 'who', 'ask', 'answered'],
      properties: { when: { type: 'string' }, channel: { type: 'string' }, who: { type: 'string' }, ask: { type: 'string' },
        answered: { type: 'boolean' }, evidence: { type: 'string' } } } },
    // deal_watch: deal/pipeline lists posted by bots that ping roles, not him (config identityIds.discord.watchPosts)
    deal_watch: { type: 'array', items: { type: 'object', required: ['source', 'deals'],
      properties: { source: { type: 'string' }, posted: { type: 'string' }, deals: { type: 'array', items: { type: 'string' } },
        unchanged_since: { type: 'string' } } } },
    decisions: { type: 'array', items: DECISION },
    ledger_rows_written: { type: 'number' }, failures: { type: 'array', items: { type: 'string' } } } }
...
const MEET_SCHEMA = { type: 'object',
  required: ['meetings_processed', 'summaries', 'not_captured', 'commitments_ledgered', 'decisions', 'prep_blocks', 'projects_touched', 'open_actions', 'failures'],
  properties: { meetings_processed: { type: 'array', items: { type: 'string' } },
    ...
    // open_actions (2026-09-14, he had 15 actions from 6 meetings and was prompted on none): every action he owns from
    // ALL of today's minute files, not just this window's meetings, resolution-checked. Rendered uncapped in YOU OWE.
    open_actions: { type: 'array', items: { type: 'object', required: ['meeting', 'action', 'due', 'done'],
      properties: { meeting: { type: 'string' }, when: { type: 'string' }, action: { type: 'string' },
        due: { type: 'string' }, done: { type: 'boolean' }, evidence: { type: 'string' } } } },
...
LOG ${OWNER}'S SENT EMAIL FOR VOICE CARDS (2026-09-25): each email ${OWNER} sent that you open in an in:sent check goes into ${VAULT}/_usage/local.nosync/sends/inbox.jsonl as {"channel":"email","chat":"<subject>","to":"<first recipient name>","ts":"<ISO time>","text":"<only what he wrote: greeting to sign-off, quoted history dropped>"} via a python3 json.dumps one-liner. Only mail you already opened.
...
BALL-ON-US DETECTION (2026-09-16: a client replied on a project alias asking a teammate for a change and it sat a week because the sweep dropped it as "not his"). ${ALIAS_PATTERN ? `The team runs one group alias per project (${ALIAS_PATTERN}) and ${OWNER} is a member, so its mail lands in his inbox even when a teammate is the addressee.` : `Shared project mailboxes and client threads land in ${OWNER}'s inbox even when a teammate is the addressee.`} For each project alias or client thread in the sweep window, find the newest real message. When it is from a client domain, carries a question/ask/direction, and has NO ${TEAM_ADDR} reply after it, the project's ball is on ${ORG}: emit one ball_on_us entry {area, owes (what ${ORG} owes, specific), owner (the teammate who should answer, or ${OWNER}), since (the client message date), venue (the alias/thread), yours (true ONLY if ${OWNER} himself owes the next move)}. Also emit ball_on_us for his OWN promised-but-undelivered commitments to a client (his sent-mail "will send X by <date>" with no delivery evidence), owner ${OWNER}, yours:true. A pure FYI or "thanks, noted" from the client does NOT move the ball; it is a read_summary line. Run the cross-surface resolution check first: a reply from ANYONE on the team after the client's message clears the ball. Never emit a ball off a thread you did not actually read this run. Empty array is fine when every client thread's last move is genuinely the client's.`,
...
STAGE: CHATS. FULL CHANNEL COVERAGE EVERY RUN (2026-08-12, his rule: "every scan should scan all the tools, all messaging channels, not just start and end"). Every mode including CAPTURE sweeps ALL of: Discord - SERVER-WIDE SEARCH ONLY: authored (authorId ${DISCORD.authorId}) + mentions (mentions ${DISCORD.authorId}) on guild ${DISCORD.guildId}, limit 25 (the API max, 50 is rejected), sortBy timestamp desc, and PAGE with offset 25, 50, 75... until the oldest hit on a page predates the run window. One page is never coverage (2026-09-14: a single 25-hit page plus digesting dropped most of 41 tags). Then a ~50-message context window around each hit (widened from 20, 2026-08-26). ${WATCH_TEXT ? `WATCHED POSTS (2026-09-14, he could not see the deals waiting on review): for each of ${WATCH_TEXT}, fetch the two newest posts by that author in that channel (discord_search_messages with channelId + authorId, limit 2; the list lives in the embed) and return deal_watch: every deal named, and unchanged_since = the older post's date when both lists are identical. These ping roles, not him, so the mention search never sees them.` : ''} TEAMS FULL COVERAGE (2026-09-15, 14 of 50 chats failed Graph side, page 2 hit a 429, channels never swept): page teams_list_chats to the end; any chat that fails or returns 429 goes on a retry list and is retried up to 3 times after the other chats are done (do other surfaces in between so the rate limit cools); then teams_list_teams -> teams_list_channels -> teams_list_channel_messages for every channel inside the window. KNOWN BLOCKER (2026-09-16): teams_list_teams returns 403 "Missing scope 'Team.ReadBasic.All'" until an M365 admin grants that scope to the connector in Entra. When it does, cover channels AND the 429-failed chats with the full-text chat_message_search (not throttled, spans all chats and channels): search "${OWNER}", their first name variants, and common words ("the", "thanks", "okay") inside the window, open each hit's chat. Report it as "channels via keyword search, scope pending", never as a fresh failure. Coverage line must state chats read N of M and channels read N of M, plus each chat still failing after retries with its exact error. TEAMS FALLBACK (2026-09-14, NAP lives on Teams and went invisible): if no Microsoft Teams tool loads, search Gmail from:no-reply@teams.mail.microsoft inside the window and treat each notification as a Teams message (who, chat, snippet, open the thread for the body). Report Teams as "via notification email", never as unswept when those emails were read. NEVER read Discord channel by channel, never walk a watchlist, never touch discord-state.md (his rule 2026-08-16). Reading a named channel is project research for a PM/project dig only, never triage. A 403 on some channel is NOT a coverage gap and never appears in the brief; the only Discord failure worth reporting is the server-wide search itself failing, Telegram all-unreads with project carve-outs, WhatsApp via whatsapp-ro, iMessage unreads, Google Chat (resolve space IDs first), Messenger via the logged-in Chrome (actually attempt it, start the browser session if needed - "not attempted" is a failure now, not a default). Page text only returns the thread that is already open, so it is never coverage (2026-09-15: only Chiefs + Finance got read, TOHP and the rest skipped): read the left chat list first (read_page/find on the sidebar, scroll it), then open EVERY thread with a message inside the window one by one via its /t/<id> link and read it. If Control Chrome says "Google Chrome is not running", switch to claude-in-chrome; if that needs a browser pick, use list_connected_browsers + select_browser on the one holding a messenger.com tab. Coverage line: threads opened N of M active. List every channel group you actually swept and every one that failed with the exact error - an unswept surface is never "quiet". Capture his own sent messages TWO ways: (a) voice learning; (b) SELF-COMMITMENTS - any promise he made ('will send today', 'I'll get this tomorrow', 'by EOD', a promised deliverable/date) becomes a ledger row owner=${OWNER} with the deadline resolved to a real date. These are verified FIRST next run: no evidence he delivered by the deadline = a NOW-lane red + chip, fired BEFORE the recipient chases him (assistant.md 'His own promises'). Decision-shaped asks go in the decisions field - but FIRST ls ${VAULT}/_decisions/ + grep ledger for 'decision framed'; already-framed decisions are never re-emitted.
...
POPULATE direct_tags EVERY run (2026-09-14, his flag: "so many tags in discord for me to review but i wasnt prompted"): one row per message in the window that @-tags ${OWNER} directly - every Discord mention hit across all pages, plus Telegram/WhatsApp/GChat/Teams messages that @ or name him with an ask. when = Manila time; channel = #name or chat name; who = display name; ask = what they want in <=14 plain words (a bare tag with no text: read the context window and say what it points at). answered = true ONLY if he replied after it in that channel/thread or answered the same ask on another surface, with where in evidence. A bot "looked for you" notice is not a tag. A tag also emitted as an item still gets its direct_tags row. Never fold a tag into a digest in place of a row.
LOG ${OWNER}'S SENDS FOR VOICE CARDS (2026-09-25): every message ${OWNER} himself sent that you already read this run on Telegram, Discord, Google Chat or Teams (WhatsApp is read locally, skip it) goes into ${VAULT}/_usage/local.nosync/sends/inbox.jsonl, one JSON line per message {"channel":"telegram|discord|gchat|teams","chat":"<chat name>","to":"<person if a DM, else empty>","ts":"<ISO time>","text":"<exactly as sent>"}, written with a python3 json.dumps one-liner. Only what you already opened; never read extra to fill it. Duplicates are fine, the builder dedupes.
BALL-ON-US in client chats (2026-09-16, same rule as the email sweep, for chat venues). For each CLIENT-facing chat group (a client WhatsApp/Telegram/Teams group or client chat space, not an internal channel), check the newest real message. When it is from the client side, carries a question/ask/direction, and no one on the team has answered after, the project's ball is on ${ORG}: emit a ball_on_us entry {area, owes, owner, since, venue, yours}. Run the cross-surface resolution check first (a team reply on ANY surface clears it). A client venue you could not read this run is named in channels_failed and produces NO ball, never a guessed one. An FYI/ack from the client does not move the ball.`,
...
OPEN ACTIONS, WHOLE DAY (2026-09-14, his flag: "so many action points from the meetings i had today but i was not prompted"). The window only covers meetings since the last run, so the morning meetings' actions never reached the evening brief. EVERY run: read EVERY minute file in ${VAULT}/_meetings/ dated today (on MORNING, yesterday's too), not only the ones you processed, and collect every Actions row owned by ${OWNER}, shared rows like "every PM (${OWNER} for X)" included. For each, run the resolution check (Gmail sent, his Discord/GChat/Telegram posts, Jira, ledger strikes, standing corrections) and return open_actions {meeting, when, action, due, done, evidence}: due resolved to a real date ("today" = the meeting date), done=true only with evidence. Make sure each has a ledger row (owner ${OWNER}).
...
- Boards: ${BOARDS_TEXT}. Load each connector via ToolSearch.${BROWSER_BOARDS_TEXT ? '\n' + BROWSER_BOARDS_TEXT : ''}
...
// ball_on_us (2026-09-16): teammate-owned balls (yours:false) lead their BY AREA block; the owner's own (yours:true) go to NEEDS YOU.
const ballOnUs = S.flatMap(r => r.ball_on_us || [])
const hisBalls = ballOnUs.filter(b => b.yours)
...
REVIEW TAGS (2026-09-17): a tag asking ${OWNER} to review a Google Doc or Sheet is ALWAYS chipped, never listed as a decision. Put every link with its file ID, plus the chat channel/thread ID and message ID, in the prompt. Recommended action: "review inside the file per the preamble's REVIEW ASKS rule (stances first, then suggestions + comments as the owner)". This stage itself never opens or edits the file.
...
const movedAnything = allItems.length || ballOnUs.length || readSummary.length || channelDigests.length || pmChanges.length || preps.length || openTags.length || openActions.length
...
Sections: 🎯 NEEDS YOU · 🏷️ TAGGED · 📝 YOU OWE · 💼 DEALS · ⚖️ DECISIONS · 🗂️ BY AREA · ✅ CLEARED · 🌙 DAY RECAP · 🔧 FIX NEXT RUN.
...
1. **Header, one line:** "as of <time>, <N> need you, <N> tagged, <N> you owe, <N> areas moved, <N> FYIs cleared. reply 'now' to refresh".
2. **🎯 NEEDS YOU** (the only must-act list - merge urgentReds + soon into ONE ranked list, max 6, drop the NOW/SOON/LATER lane labels entirely): each = one line, "who/what - the ask - deadline" and end "-> chip" if it has one. This is the section he acts on; keep it at the very top and tight. If more than 6 genuinely need him, the tail were probably actions the system should progress - note "+N more in the ledger" rather than listing. Also fold in his_balls: a ball on ${ORG} that ${OWNER} owes the next move on (yours:true), e.g. a client reply awaiting his answer or his own promised-but-undelivered deliverable, is a NEEDS YOU line ("<area>: <what he owes> - client waiting since <since>"). Teammate-owned balls do NOT come here; they render in BY AREA.
...
6b. **🎙️ MEETINGS** (every mode, from meeting_summaries; omit the section entirely if empty). His ask 2026-08-20: summarise every meeting each triage, full minutes stay in the vault. One block per meeting, newest first, max 8: bold title + time on one line, then the gist as ONE or TWO bullets. His actions live in 📝 YOU OWE, not here. NO attendee lists, NO ticket numbers, NO agenda replay. End the section with one line: "minutes: _meetings/" - never paste minute content into the brief. A meeting whose only content is "nothing moved" gets a single bullet, not a block.
...
ball_on_us=${JSON.stringify(ballOnUs.slice(0, 25))}
his_balls=${JSON.stringify(hisBalls.slice(0, 10))}
...
${mode === 'CAPTURE' ? `THIS IS A MIDDAY COMPACT UPDATE (12/3/5pm) - even tighter: Header line, then 🎯 NEEDS YOU (only if any), then 🏷️ TAGGED and 📝 YOU OWE in full (only if any, never compacted), then a max-5-line 🗂️ SINCE LAST RUN merge of the biggest area moves (area emoji per line), then the FULL ✅ CLEARED list. "Compact" tightens the AREA merge, NEVER the cleared list: render EVERY FYI from cleared_fyis as its own named line with its specifics, exactly as section 5 above - a one-line "Cleared: N FYIs" count is a HARD-RULE VIOLATION (2026-08-13, he flagged it again 2026-08-17: "it didnt even summarize what those FYIs are"). The count is never a substitute for the items. SKIP Decisions, full By-area, Day-recap, Sharper-tomorrow. Nothing moved -> one line "All quiet since <last run>, nothing needs you." Keep the section emojis. The area merge is skimmable in 15 seconds; the cleared list is as long as it needs to be.` : ''}
