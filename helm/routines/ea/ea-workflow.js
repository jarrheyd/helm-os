export const meta = {
  name: 'ea-run',
  description: 'EA scheduled run: parallel sweeps with schema-enforced returns; brief or receipt composed from structured data so contract fields cannot be skipped',
  phases: [
...
if (!mode || !now) {
  const clock = await agent(`Determine the EA run's mode and time. Steps:
...
const COMMON = `You are ONE stage of ${OWNER}'s EA run (mode ${mode}, now ${now} Manila, window: ${since}).
...
- The EA may only create and edit drafts it authored itself, and never a second draft on a thread that already has one of his.
...
3. Then ONE PushNotification, under 200 chars, no markdown, leading with the nearest deadline: e.g. "EA: Nu deploy call by 6pm + 2 more - chips in app". Skip it entirely if nothing is due before the next brief.
...
3. Generate speech with the SANCTIONED helper - do NOT hand-roll TTS. Write your finished (deslopped) script to a temp file, then run exactly (the --prompt sets the on-air delivery, keep it):
     python3 "${VAULT}/_meta/ea-tts.py" --text <script.txt> --voice Charon --prompt "${audioStyle}" --out "${VAULT}/_audio/${now.slice(0,10)}-${slot}.m4a"
...
4. REBUILD THE PLAYER PAGE: run
   python3 "${VAULT}/_meta/build-audio-player.py" "${VAULT}/_audio/${now.slice(0,10)}-${slot}.m4a" ${mode} --dek "<one plain sentence summarizing the episode>" --rows "<'Name|note' pairs separated by ';' - one per major thread, e.g. 'Needs you|1 red · 5 yellow;Decisions|10 of 53 open;By area|10 moved'>"
   It writes ${VAULT}/_audio/player.html. Then publish that file with the Artifact tool, passing url "${AUDIO_ARTIFACT_URL}" so the STANDING player URL updates in place (never publish without url - that would mint a new link). favicon "🎧", description "Your nightly EA audio brief, playable from any phone or laptop with lock-screen controls and resume."
5. FAIL-SOFT at every step: if the ea-tts.py helper is missing or exits nonzero, return audio_path:'' with the reason (do NOT fall back to a direct API call); if the player rebuild or republish fails, still return the mp3 path and note the failure in reason - never break the brief.`,
