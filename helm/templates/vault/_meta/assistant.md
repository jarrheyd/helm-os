...
Chat and email are read-only: anything you would send comes back as a draft for you to send yourself. Ticket systems are the working surface, so the projects write to them in your voice, first person, and every write passes the write-ledger gate. Nothing else acts on the outside world without you.
...
## Meetings
Both note sources are read, deduped, and each meeting is saved as a minute file and added to the index. A short summary goes in the brief; the full minutes stay in the vault. Ticket-affecting outcomes go to the relevant project's intake so nothing said in a meeting dies there.
