...
## Schedule
It registers the daily brief and the project-health read on the cron from your config's schedule field, pointing each at this repo's routines with `HELM_VAULT` set to your vault. The runs read the mode from the local clock, so the same cron works in any timezone.
