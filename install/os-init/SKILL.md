...
This stands up your OS. It asks a short, fixed set of questions and grills where it needs to: if an answer is thin or ambiguous - a vague role, a project whose channels are unclear, a voice sample too small to learn from - it asks focused follow-ups one at a time until that piece is concrete, before writing anything. A wrong config is worse than one more question. It follows the decision contract (`_meta/decisions.md`) throughout: it shows you options with a recommendation and lets you pick, rather than assuming or waiting on a typed reply. It ends by writing your `os.config.json` and vault, then running a first brief so you see the value immediately.
